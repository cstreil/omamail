...
Item {
  ...
  // Kept behind the Service facade so every view remains protocol-neutral.
  readonly property bool contactsDirectoryAvailable: backendCanListContacts
    && activeAccountId !== ""
  ContactDirectory {
    id: contactDirectory
    backend: root.backend
    available: root.contactsDirectoryAvailable
    activeAccountId: root.activeAccountId
  }
  readonly property var contactSources: contactDirectory.sources
  readonly property string contactSource: contactDirectory.sourceId
  readonly property var contactRows: contactDirectory.rows
  readonly property int contactTotal: contactDirectory.total
  readonly property bool contactsDirectoryBusy: contactDirectory.busy
  readonly property var openContact: contactDirectory.openContact
  readonly property string contactsDirectoryError: contactDirectory.error
  readonly property string contactsDirectoryDetailError: contactDirectory.detailError
  readonly property string contactsDirectoryAccountId: contactDirectory.accountId
  readonly property string contactQuery: contactDirectory.query
  function contactErrorText(error) { return contactDirectory.errorText(error) }
  function refreshContactSources(accountId, query) {
    contactDirectory.refreshSources(accountId, query)
  }
  function refreshContactList(accountId, query) {
    contactDirectory.refreshList(accountId, query)
  }
  function selectContactSource(sourceId, query) {
    contactDirectory.selectSource(sourceId, query)
  }
  function openContactDetail(contactId) { contactDirectory.openDetail(contactId) }
  function closeContactDetail() { contactDirectory.closeDetail() }

...
// The places a ready account can start. Switching between them is `replaceRoot`,
// never a push: calendar and contacts are not somewhere the user went from the
// list, they are other things the window is for.
var ROOTS = ["list", "calendar", "contacts"]
...
// to draw.
var KINDS = ["list", "calendar", "contacts", "reader", "calendarDetail", "settings",
...
function rootFor(state) {
  ...
  if (value.anyReady === true) {
    ...
    return [entry(contains(ROOTS, view) ? view : DEFAULT_KIND)]
  }

...
Google Calendar uses the same sign-in, but the **Google Calendar API** must also be enabled in the same Google Cloud project as the Gmail OAuth client, alongside Gmail API. Enable it in the [API library](https://console.cloud.google.com/apis/library/calendar-json.googleapis.com) or run `gcloud services enable calendar-json.googleapis.com --project=PROJECT_ID`, replacing `PROJECT_ID` with that OAuth project's ID. If calendar requests fail, check the API status for that project; the native calendar may report a generic request failure.

Omamail currently reads and writes the account's **primary Google calendar** only. Events that exist only on secondary, shared or delegated calendars, including Google's separate Birthdays calendar, do not appear in this view; view them in a calendar client that supports those calendars.
